        ...
        if (test) {
            ...
            if (printToConsole && (primaryOnly && isPrimary || !primaryOnly)) {
                if (test.status !== 'OFF') {
                    ...
                    console.log(`  ${test.id}. ${primaryOnly ? '' : isPrimary ? 'p' : 'w'}TEST > ${endLengthening(test.name, 50)} ${startLengthening(test.time_ms, 8)}ms ${status} ${err ? '-> ' + err : ''}`);
                }
            }
        }
        // continue
